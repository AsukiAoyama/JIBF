...
function OpenModeSettings(){
    ...
    if(stateString === "Einfach-Taster"){
      ...
      CenterSwitch.addEventListener("click", switchRelayOnce);


      function switchRelayOnce() {
        if (!switchRelayOnce.clicked) {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/SwitchRelay1', true);
            xhr.send();
    
            switchRelayOnce.clicked = true; // Set the flag to true after the first click
        }
    }
